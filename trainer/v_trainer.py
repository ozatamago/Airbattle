...
class ValueFunctionTrainer:
    def __init__(self, v, learning_rate=1e-3, gamma=0.99):
        self.v = v
        self.optimizer = optim.Adam(v.parameters(), lr=learning_rate)
        self.gamma = gamma

    def calculate_target(self, rewards, next_states, dones, v, gamma=0.99, lambda_gae=0.95):
        y_lambda = torch.zeros_like(rewards)
        next_value = v(next_states[-1]).detach()
...
# 学習ループの例
# # 価値関数ネットワークのインスタンス化
# v = ...  # 価値関数ネットワークのインスタンス化

# # トレーナークラスのインスタンス化
# trainer = ValueFunctionTrainer(v)
